...
bool ship::within_ship(pos p) const {
    return ((direction == "horizontal" && matches_vertical(p) &&
             within_horizontal(p))

            ||

            (direction == "vertical" && matches_horizontal(p) &&
             within_vertical(p)));
}
...
void ship::add_hit_to_ship(pos p) {
    if (direction == "horizontal")
        hits.at(p.x - start_pos.x) = 1;
    else
        hits.at(p.y - start_pos.y) = 1;
}
...
void ship::print() const {
    cout << "c: " << c << " length: " << length << " start x: " << start_pos.x
         << " start_y: " << start_pos.y << " dir: " << dir << " is sunk? "
         << is_sunk() << endl;
}
